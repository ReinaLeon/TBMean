...
const registerUser = async (req, res) => {
  ...
  const userRegister = new user({
    ...
    roleId: roleId._id, //el role id que le asignamos
    dbStatus: true,
  });
...
const updateUser = async (req, res) => {
  if (!req.body.name || !req.body.email || !req.body.roleId)
    //si no esta el password aca, es porque es un dato obligatorio
    return res.status(400).send({ message: "Incomplete data" });

  const changeEmail = await user.findById({ _id: req.body._id });
  if (req.body.email !== changeEmail.email)
    //el correo que llego es diferente al que esta dentro del json de la base de datos
    return res
      .status(400)
...
const login = async (req, res) => {
  ...
  if (!userLogin) return res.status(400).send({ message: "Wrong email" }); //wrong equivocado
